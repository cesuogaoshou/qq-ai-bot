...
当前已完成 M1 本地消息闭环、M2 @ 触发大模型回复、M2.5 搜索/安全/成本基础框架、M3 管理员控制、M4 SQLite 消息持久化和最近聊天总结基础能力。下一阶段进入 M5 图片和多模态评估。
...
图片能力不会直接并入普通聊天链路。读图优先于出图，且必须满足显式请求、管理员可关、每日限额和默认不长期保存原图。当前默认文本模型仍保持 `doubao-seed-2.0-lite`；视觉模型和图片生成模型需要在确认火山方舟官方能力、价格和配额后再接入。
