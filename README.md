# QQ 群混合型大模型机器人

这是一个面向 QQ 群聊场景的混合型大模型机器人方案。目标不是做一个高频问答助手，而是做一个“轻量群友 + 可控助手”：平时不抢话，被 @ 或明确请求时认真回答，管理员可以控制开关、频率和后续上下文能力。

当前已完成 M1 本地消息闭环、M2 @ 触发大模型回复、M2.5 搜索/安全/成本基础框架、M3 管理员控制、M4 SQLite 消息持久化和聊天总结、M5 读图输入，以及云服务器长期运行部署。联网搜索已接入 Tavily 适配器；每日 09:00 自动总结昨天群聊；图片生成和语音转文字仍后置。

## 当前结论

推荐采用保守 MVP 路线：

```text
单 QQ 群灰度
  -> OneBot 消息闭环
  -> @ 触发大模型回复
  -> 管理员控制、限频、日志
  -> 关键词和冷场接话体验优化
  -> 多群配置和长期运行优化
```

第一版先支持一个固定群。数据模型和配置结构预留 `group_id`，但不在 MVP 中实现完整多群配置管理。这样可以先验证稳定性、群聊体验和账号风险，再扩展到多个群。

## 技术方向

- QQ 接入：NapCatQQ 当前运行，Lagrange.OneBot 作为备选。
- 通信协议：OneBot WebSocket 优先，HTTP API 用于发送动作。
- 后端语言：Python 3.12。
- 存储：SQLite。
- 大模型：先接远程 API，后续再考虑本地模型。
- 部署：本地开发 + 阿里云 ECS 长期运行。

当前长期运行方案为 Ubuntu 22.04 ECS：NapCat Shell 版负责 QQ/OneBot 协议端，`qq-ai-bot` 由 systemd 常驻，SQLite 存储在服务器本地 `data/bot.sqlite3`。

## 文档

- [产品规格书](docs/product-spec.md)：产品定位、MVP 范围、交互策略、成功标准。
- [技术选型设计](docs/technical-selection.md)：关键技术选择、取舍和风险。
- [架构设计](docs/architecture-design.md)：模块边界、数据模型、流程和配置草案。
- [路线图](docs/roadmap.md)：M1 到长期阶段的推进顺序。
- [开发规约](docs/development-guidelines.md)：代码边界、测试、安全、日志和配置约定。
- [高级能力规格](docs/advanced-capabilities-spec.md)：联网搜索、记忆、安全、多模态和成本控制。
- [M1 本地联调手册](docs/m1-local-runbook.md)：OneBot 协议端接入、环境变量、启动和排错步骤。
- [M2 本地联调手册](docs/m2-local-runbook.md)：@ 触发大模型回复、上下文和常见问题。
- [M2.5 本地运行手册](docs/m2-5-local-runbook.md)：搜索触发、安全分级和预算控制。
- [M3 本地运行手册](docs/m3-local-runbook.md)：管理员开关、状态查看、SQLite 和限频验证。
- [M4 本地运行手册](docs/m4-local-runbook.md)：聊天记录持久化、最近总结、记忆状态和清理。
- [M5 图片和多模态评估](docs/m5-multimodal-evaluation.md)：读图、出图、模型能力、成本和隐私边界评估。
- [M6 语音能力评估](docs/m6-voice-evaluation.md)：QQ 语音、OneBot record、转码、ASR 成本和放弃条件。
- [服务器长期运行配置手册](docs/server-deployment.md)：ECS、NapCat、systemd、日志、备份和常见运维命令。

## 本地开发

```powershell
python -m venv .venv
.\.venv\Scripts\python.exe -m pip install -e ".[dev]"
.\.venv\Scripts\python.exe -m pytest
```

本地运行前复制 `.env.example` 为 `.env`，并填写 OneBot、大模型和目标群配置。

M1/M2 本地运行：

```powershell
$env:ONEBOT_WS_URL="ws://127.0.0.1:3001"
$env:ONEBOT_HTTP_URL="http://127.0.0.1:3000"
$env:BOT_QQ="机器人 QQ 号"
$env:TARGET_GROUP_ID="你的测试群号"
$env:LLM_API_KEY="你的 API Key"
.\.venv\Scripts\qq-ai-bot.exe
```

在目标群发送 `/bot ping`，预期机器人回复 `pong`。@ 机器人时，如果已配置 `LLM_API_KEY`，会调用大模型生成回复。

M3 管理员命令：

```text
/bot on
/bot off
/bot status
/bot summary recent
/bot memory status
/bot memory clear
/bot summary clear
```

管理员 QQ 号通过 `BOT_ADMIN_QQ_IDS` 配置，多个 QQ 号用英文逗号分隔。

M4 会把目标群内通过安全策略的消息写入 SQLite。隐私或高风险请求不会进入持久化记忆；总结前会对手机号、身份证、银行卡和住址类内容做规则脱敏。

## 服务器运行

当前服务器长期运行依赖两个服务：

```text
NapCat Shell 版
  -> 127.0.0.1:3000 HTTP Server
  -> 127.0.0.1:3001 WebSocket Server

qq-ai-bot systemd 服务
  -> /opt/qq-ai-bot/qq-ai-bot
  -> /opt/qq-ai-bot/qq-ai-bot/data/bot.sqlite3
```

常用运维命令：

```bash
systemctl status qq-ai-bot --no-pager
journalctl -u qq-ai-bot -f
systemctl restart qq-ai-bot

systemctl status napcat --no-pager
journalctl -u napcat -f
systemctl restart napcat
```

服务器部署细节见 [服务器长期运行配置手册](docs/server-deployment.md)。

## M2.5 能力状态

当前默认文本模型仍是火山方舟 `doubao-seed-2.0-lite`。

M2.5 增加的是策略层、接口层和 Tavily 搜索适配器：

- 安全分级：正常问题继续回答；隐私、高风险和骂战请求会被拦截或降温。
- 搜索触发：识别“搜一下、查一下、最新、今天、现在”等请求。
- 成本控制：搜索按群和用户做每日次数限制。
- 搜索工具：默认关闭；配置 `ENABLE_WEB_SEARCH=true`、`WEB_SEARCH_PROVIDER=tavily` 和 `TAVILY_API_KEY` 后才会调用真实搜索。

Tavily 适配器使用基础搜索深度，适合先做低成本灰度。真实使用前仍需要你在供应商控制台确认价格、配额和 key 权限。

## M5 读图状态

当前已经具备读图输入基础能力：

- 可解析 OneBot 图片段。
- 支持 `@机器人 + 图片` 默认描述图片。
- 支持“看图、读图、图中、图里、这张图、提取文字、识别文字”等明确读图请求。
- 支持最近图片短期缓存，用户先发图再 @ 机器人提取时可尝试使用同一用户最近图片。
- 图片内容不写入 SQLite 聊天记忆，不作为长期用户画像。
- `/bot status` 会显示图片开关、模型、群/用户每日限额、最大图片大小和图片缓存状态。

图片能力仍默认可关闭、可限额。`IMAGE_INPUT_MODEL` 为空时复用 `LLM_MODEL`；如果后续改用专门视觉模型，再单独配置。图片生成暂未实现。

## M6 语音评估状态

语音能力暂不进入代码实现。OneBot v11 有 `record` 语音消息段和 `get_record` API，但实际可用性依赖当前协议端、语音 URL 或文件路径、ffmpeg 转码环境和 ASR 服务价格。下一步应先在真实群里观察语音事件结构，再决定是否进入 M6a 协议端探针。

## MVP 成功标准

- 能稳定接收目标群消息。
- 被 @ 后能在合理时间内回复。
- 普通消息默认不触发回复。
- 管理员能开启、关闭和查看状态。
- 每群、每用户限频生效。
- 大模型超时或发送失败时不会导致进程崩溃。
- 日志能定位连接、触发、模型调用和发送失败原因。
- 云服务器上 NapCat 和 `qq-ai-bot` 均可由 systemd 常驻运行。

## 非 MVP 范围

- 图片理解、语音识别、表情包回复。
- Web 管理后台。
- 多平台接入。
- 复杂插件市场。
- 自动踢人、禁言等群管理动作。
- 长期复杂用户画像。
- 完整多群配置管理。
