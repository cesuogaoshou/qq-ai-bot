...
OpenAI 兼容接口只是协议形态，不表示默认供应商是 OpenAI。
...
联网搜索、读图、图片生成和语音转文字不直接进入默认主链路。它们必须满足：
...
1. 文本对话继续使用 `doubao-seed-2.0-lite`。
2. 联网搜索先做工具接口和触发策略，具体供应商接入前再确认价格。
3. 读图优先于出图。
4. 语音转文字后置，取决于协议端语音文件获取稳定性。
